    ...
    <div className={`group relative bg-white rounded-2xl p-4 shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 flex flex-col ${priority ? 'animate-pulse-subtle' : ''}`}>
      ...
      <div className="absolute top-3 left-3 z-10 flex flex-col gap-1">
        ...
        {product.deliveryToday && (
          <span className="bg-blue-900 text-white text-[9px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
            <Truck className="h-3 w-3" /> Cascavel Express
          </span>
        )}
      </div>
      ...
      <div className="flex-grow">
        ...
        <Link to={`/product/${product.id}`} className="block">
          <div className="flex flex-col">
            <h3 className="text-sm font-semibold text-gray-800 leading-snug line-clamp-2 h-[2.5rem] group-hover:text-red-500 transition-colors">
              {product.name}
            </h3>
            {product.hasFreeShipping && (
              <div className="flex items-center gap-1 mt-1">
                <span className="inline-flex items-center gap-1 text-emerald-600 font-black text-[10px] uppercase tracking-tighter bg-emerald-50 px-1.5 py-0.5 rounded">
                  <Truck className="h-3 w-3" />
                  Frete Grátis*
                </span>
                <span className="text-[8px] text-gray-400 font-bold uppercase italic">Algumas regiões</span>
              </div>
            )}
          </div>
        </Link>
      ...
      <div className="mt-4">
        {isOutOfStock ? (
          <Link 
            ...
            className="w-full bg-gray-100 text-gray-600 py-3 rounded-xl text-xs font-bold flex items-center justify-center gap-2"
          >
            Avisar Disponibilidade
          </Link>
        ) : (
          <button 
            onClick={() => onAddToCart(product)}
            className="w-full bg-red-500 text-white py-3 rounded-xl text-xs font-bold flex items-center justify-center gap-2 hover:bg-red-600 active:scale-95 transition-all shadow-md"
          >
